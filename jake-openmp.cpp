...
void simulate_one_step(particle_t* parts, int num_parts, double size) {
    ...
    #pragma omp barrier
    #pragma omp for
    for (int cell_id = 0; cell_id < grid.size(); cell_id += 1) { // Loop over every cell
        ...
        for (int p_id : grid[cell_id].particles) { // Loop over every particle in that cell
            ...
            for (int row_change = -1; row_change <= 1; ++row_change) { // Iterate over all 8 neighboring cells and current cell
